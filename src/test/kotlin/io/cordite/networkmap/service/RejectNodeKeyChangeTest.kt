...
import io.cordite.networkmap.utils.NMSUtil.Companion.createAliceSignedNodeInfo
...
class RejectNodeKeyChangeTest {
	companion object {
		val log = loggerFor<RejectNodeKeyChangeTest>()
		@JvmField
		@ClassRule
	...
	fun `that we can register node twice with same legal name but different keys`(context: TestContext) {
		...
		internalDriver(
			...
			//NetworkMap does not have two nodes as network map rejected publish nodeInfo when the node with the same legal name tried to publish with different key
			context.assertTrue(nmNodes.size!=nodeNodes.size)
			}
	}
	
	@Test
	fun `that we can not test publish api for the same node name with a different key`(context: TestContext) {
		val rootCert = service.certificateManager.rootCertificateAndKeyPair.certificate
		val nmc = createNetworkMapClient(rootCert, PORT)
		
		val sni1 = createAliceSignedNodeInfo(service)
		...
		val sni2 = createAliceSignedNodeInfo(service)
		...
		try {
			...
			NMSUtil.waitForNMSUpdate(vertx)
			assertEquals(1, nmc.getNetworkMap().payload.nodeInfoHashes.size)
